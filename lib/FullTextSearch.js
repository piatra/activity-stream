...
const SQL_SELECT_MATCH_TYPE = `SELECT url, title, type, description, snippet(fts_pages) AS snippet, last_visit
  FROM fts_pages WHERE fts_pages MATCH :keywords AND type = :type ORDER BY last_visit DESC`;
...
FullTextSearchStore.prototype = {
  ...
  asyncSearchPagesExact: Task.async(function*(keywords, type) {
    let results;

    if (type) {
      results = yield this.asyncExecuteQuery(
        SQL_SELECT_MATCH_TYPE,
        {
  ...
  asyncSearchGeneric: Task.async(function*(options) {
    let queryTasks = [];
    let {keywords, source, type} = options;
    let sources;
    if (source === undefined || source === "all") {
      sources = ["pages", "history", "bookmarks", "tabs"];
    } else {
      sources = [source];
    }
    ...
    for (let source of sources) {
      switch (source) {
        ...
        case "history":
          ...
          break;
        case "pages":
          queryTasks.push(this.asyncSearchPagesExact(keywords, type));
          break;
        case "tabs":
