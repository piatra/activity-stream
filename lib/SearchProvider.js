...
const {FTSRawPages} = require("lib/FTSRawPages.js");
...
NewTabSearchProvider.prototype = {
  ...
  init() {
    ...
    Task.spawn(function*() {
      ...
      yield this._fullTextSearchStore.asyncInsert(FTSRawPages);
    }.bind(this));
  },
  ...
  getFilteredActivity: Task.async(function*(searchString) {
    /*
    return this._fullTextSearchStore.asyncSearchPagesExact(searchString).then(pages => {
        console.log("PAGES:" + JSON.stringify(pages));
        return pages;
    });
    return this._fullTextSearchStore.asyncSearchBookmarks(searchString).then(bookmarks => {
        console.log("Bookmarks:" + JSON.stringify(bookmarks));
        return bookmarks;
    });
    return this._fullTextSearchStore.asyncSearchHistory(searchString, 50).then(history => {
        console.log("History:" + JSON.stringify(history));
        return history;
    });
    return this._fullTextSearchStore.asyncSearchTabs(searchString).then(tabs => {
    ...
    */
    let options = {
      ...
      sources: ["pages", "tabs", "bookmarks", "history"]
    };
    return this._fullTextSearchStore.asyncSearchGeneric(options).then(results => {
        console.log("SEARCH ALL:" + JSON.stringify(results));
        return results;
    });
  }),
