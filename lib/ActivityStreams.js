...
ActivityStreams.prototype = {
  ...
  _loadRecommender() {
    ...
    this._memoized.getAllHistoryItems().then(historyItems => {
      ...
      this._baselineRecommender = new Recommender(historyItems, {
        simplePrefs,
  ...
  _loadWeightedHighlightsCoefficients() {
    try {
      let value = simplePrefs.prefs.weightedHighlightsCoefficients.split(" ");
  ...
  _respondToPlacesRequests({msg, worker}) {
    ...
    switch (msg.type) {
      ...
      case am.type("NOTIFY_BLOCK_RECOMMENDATION"):
        ...
        break;
      case am.type("NOTIFY_UPDATE_WEIGHTS"):
        console.log(msg);
        this._baselineRecommender.updateOptions({updateWeights: msg.data});
        break;
    }
  },
