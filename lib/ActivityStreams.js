  ...
  _respondToSearchRequests({msg, worker}) {
    ...
    switch (msg.type) {
      case am.type("FILTER_REQUEST"):
        SearchProvider.search.getFilteredActivity(msg.data).then(activity => {
          console.log("FILTERED STUFF: " + JSON.stringify(activity));
          let sources = ["pages", "history", "bookmarks", "tabs"];
          let links = [];

          // flatten the links to send to the embedly process for extra metadata
          for (let source of sources) {
            if (activity[source] && activity[source].length > 0) {
              Array.prototype.push.apply(links, activity[source]);
            }
          }
          this._previewProvider.getLinkMetadata(links).then(linksToSend => {
            // put the links in an object for easy retrieval by url
            let linksWithMetadata = {};
            for (let link of linksToSend) {

              // TODO: FIXME mock the device name for now.
              const devices = ["iPhone", "Android", "iPad", "MacBookPro"];
              link.device = devices[Math.floor(Math.random()*devices.length)];

              linksWithMetadata[link.url] = link;
            }

            // add the embedly metadata to the activity
            for (let source of sources) {
              if (activity[source] && activity[source].length > 0) {
                for (let link of activity[source]) {
                  Object.assign(link, linksWithMetadata[link.url]);
                }
              }
            }

            this.send(am.actions.Response("FILTER_RESPONSE", {activity}), worker);
          });
        });
        break;
